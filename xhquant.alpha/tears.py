...
def create_returns_tear_sheet(
    ...
    plotting.plot_returns_table(
        alpha_beta.reset_index(drop=True), mean_quant_rateret, mean_ret_spread_quant
    )

    plotting.plot_quantile_returns_bar(
        mean_quant_rateret.reset_index(drop=True),
        by_group=False,
        ylim_percentiles=None,
        ax=gf.next_row(),
    )
    ...
    if "1D" in factor_returns:
        ...
        plotting.plot_cumulative_returns(
            factor_returns["1D"].reset_index(drop=True), period="1D", title=title, ax=gf.next_row()
        )
    ...
    plotting.plot_mean_quantile_returns_spread_time_series(
        mean_ret_spread_quant.reset_index(drop=True),
    ...
    # plt.show()
    plt.savefig("Wise Return By Factor Quantile.pdf", bbox_inches='tight')
    gf.close()
...
def create_information_tear_sheet(
    ...
    # plt.show()
    plt.savefig("Information Coefficient.pdf", bbox_inches='tight')
    gf.close()
...
def create_turnover_tear_sheet(factor_data, turnover_periods=None):
    ...
    for period in turnover_periods:
        ...
        plotting.plot_top_bottom_quantile_turnover(
            quantile_turnover[period].reset_index(drop=True), period=period, ax=gf.next_row()
        )
    ...
    for period in autocorrelation:
        ...
        plotting.plot_factor_rank_auto_correlation(
            autocorrelation[period].reset_index(drop=True), period=period, ax=gf.next_row()
        )

    # plt.show()
    plt.savefig("Factor Rank Autocorrelation.pdf", bbox_inches='tight')
    gf.close()
