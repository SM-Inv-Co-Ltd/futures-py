...
def plot_turnover_table(autocorrelation_data, quantile_turnover):
    ...
    for period in sorted(quantile_turnover.keys()):
        for quantile, p_data in quantile_turnover[period].iteritems():
            turnover_table.loc["Quantile {} Mean Turnover ".format(quantile), "{}D".format(period)] = p_data.mean()
    auto_corr = pd.DataFrame()
    for period, p_data in autocorrelation_data.iteritems():
        auto_corr.loc["Mean Factor Rank Autocorrelation", "{}D".format(period)] = p_data.mean()
...
def plot_information_table(ic_data):
    ...
    ic_data.to_csv("IC_Distribution.dat", float_format="%.4f", sep="\t")
    file_object = open("IC_Summary.dat", "w")
    print("IC_Mean:\n", ic_summary_table["IC Mean"], file=file_object)
    print("IC_Std:\n", ic_summary_table["IC Std."], file=file_object)
    print("Risk-Adjusted_IC:\n", ic_summary_table["Risk-Adjusted IC"], file=file_object)
...
def plot_ic_ts(ic, ax=None):
    ...
    """
    ic = ic.copy().reset_index(drop=True)
    ...
    for a, (period_num, ic) in zip(ax, ic.iteritems()):
        ...
        ic.rolling(window=10).mean().plot(ax=a, color='forestgreen', lw=2, alpha=0.8)

        a.set(ylabel='IC', xlabel="")
        period_num = period_num.replace("D", "T")
        ...
        a.legend(['IC', '10 periods moving avg'], loc='upper right')
...
def plot_quantile_returns_bar(mean_ret_by_q,
    ...
    if by_group:
        ...
        (mean_ret_by_q.multiply(DECIMAL_TO_BPS).plot(kind='bar', title="Mean Period Wise Return By Factor Quantile", ax=ax))
        ax.set(xlabel='', ylabel='Mean Return (bps)', ylim=(ymin, ymax))

        # 小海添加
        mean_ret_by_q.multiply(DECIMAL_TO_BPS).to_csv("Quantile_Returns_Bar.dat", float_format="%.4f", sep="\t")
...
def plot_quantile_returns_violin(return_by_q, ylim_percentiles=None, ax=None):
    ...
    sns.violinplot(data=unstacked_dr, x='factor_quantile', hue='forward_periods', y='return', orient='v', cut=0, inner='quartile', ax=ax)
    ax.set(xlabel='', ylabel='Return (bps)', title="Period Wise Return By Factor Quantile", ylim=(ymin, ymax))

    ax.axhline(0.0, linestyle='-', color='black', lw=0.7, alpha=0.6)

    # 小海添加
    unstacked_dr.to_csv("Quantile_Returns_Violin.dat", float_format="%.4f", sep="\t")
...
def plot_mean_quantile_returns_spread_time_series(mean_returns_spread, std_err=None, bandwidth=1, ax=None):
    ...
    if isinstance(mean_returns_spread, pd.DataFrame):
        ...
        for (i, a), (name, fr_column) in zip(enumerate(ax), mean_returns_spread.iteritems()):
            ...
            a = plot_mean_quantile_returns_spread_time_series(fr_column, std_err=stdn, ax=a)
            ax[i] = a
            curr_ymin, curr_ymax = a.get_ylim()
            ymin = curr_ymin if ymin is None else min(ymin, curr_ymin)
            ymax = curr_ymax if ymax is None else max(ymax, curr_ymax)
    ...
    title = ('Top Minus Bottom Quantile Mean Return ({} Period Forward Return)'.format(periods if periods is not None else ""))
    ...
    mean_returns_spread_bps.rolling(window=10).mean().plot(color='orangered', alpha=0.7, ax=ax)
    ax.legend(['mean returns spread', '10 periods moving avg'], loc='upper right')
...
def plot_factor_rank_auto_correlation(factor_autocorrelation, period=1, ax=None):
    ...
    factor_autocorrelation.plot(title='{}D Period Factor Rank Autocorrelation'.format(period), ax=ax)
    ...
    ax.text(.05, .95, "Mean %.3f" % factor_autocorrelation.mean(),
            fontsize=16, bbox={'facecolor': 'white', 'alpha': 1, 'pad': 5}, transform=ax.transAxes, verticalalignment='top')
...
def plot_top_bottom_quantile_turnover(quantile_turnover, period=1, ax=None):
    ...
    turnover.plot(title='{}D Period Top and Bottom Quantile Turnover'.format(period), ax=ax, alpha=0.6, lw=0.8)
...
def plot_monthly_ic_heatmap(mean_monthly_ic, ax=None):
    ...
    mean_monthly_ic.index = pd.MultiIndex.from_arrays([new_index_year, new_index_month], names=["year", "month"])
...
def plot_cumulative_returns(factor_returns, period, freq=None, title=None, ax=None):
    ...
    ax.set(ylabel='Cumulative Returns', title=("Portfolio Cumulative Return ({} Fwd Period)".format(period) if title is None else title), xlabel='')
...
def plot_cumulative_returns_by_quantile(quantile_returns, period, freq=None, ax=None):
    ...
    # 小海添加
    cum_ret.to_csv("Cumulative_Returns_By_Quantile.dat", float_format="%.4f", sep="\t")
