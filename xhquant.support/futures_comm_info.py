...
def _futures_comm_qihuo_process(df: pd.DataFrame, name: str = None) -> pd.DataFrame:
    ...
    common_temp_df["合约名称"] = (common_temp_df["合约品种"].str.split("(", expand=True).iloc[:, 0].str.strip())
    common_temp_df["合约代码"] = (common_temp_df["合约品种"].str.split("(", expand=True).iloc[:, 1].str.strip(")"))

    common_temp_df["涨停板"] = (common_temp_df["涨/跌停板"].str.split("/", expand=True).iloc[:, 0].str.strip())
    common_temp_df["跌停板"] = (common_temp_df["涨/跌停板"].str.split("/", expand=True).iloc[:, 1].str.strip())
    ...
    temp_df_yuan = common_temp_df["手续费标准-开仓"][common_temp_df["手续费标准-开仓"].str.contains("元")]
    ...
    try:
        ...
        temp_df_ratio = np.nan
    temp_df_yuan = common_temp_df["手续费标准-平昨"][common_temp_df["手续费标准-平昨"].str.contains("元")]
    ...
    try:
        ...
        temp_df_ratio = np.nan
    temp_df_yuan = common_temp_df["手续费标准-平今"][common_temp_df["手续费标准-平今"].str.contains("元")]
...
def futures_comm_info(symbol: str = "所有") -> pd.DataFrame:
    ...
    if symbol == "上海期货交易所":
        ...
        big_df = pd.DataFrame()
        big_df = big_df.append(_futures_comm_qihuo_process(shfe_df, "上海期货交易所"), ignore_index=True)
        big_df = big_df.append(_futures_comm_qihuo_process(dce_df, "大连商品交易所"), ignore_index=True)
        big_df = big_df.append(_futures_comm_qihuo_process(czce_df, "郑州商品交易所"), ignore_index=True)
        big_df = big_df.append(_futures_comm_qihuo_process(ine_df, "上海国际能源交易中心"), ignore_index=True)
        big_df = big_df.append(_futures_comm_qihuo_process(cffex_df, "中国金融期货交易所"), ignore_index=True)
        return big_df
...
if __name__ == "__main__":
    ...
    print(futures_comm_info_df.shape)
